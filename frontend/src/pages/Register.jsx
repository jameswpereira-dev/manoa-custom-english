...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    setError('');
    if (senha !== conf) return setError('As senhas não coincidem.');
  ...
  return (
    <AuthCard title="Criar conta" subtitle="Comece sua jornada no inglês">
      ...
      <form onSubmit={handleRegister}>
        <Input label="Nome completo" type="text" value={nome} onChange={e=>setNome(e.target.value)} required placeholder="Seu nome" />
        ...
        <Input label="Confirmar senha" type="password" value={conf} onChange={e=>setConf(e.target.value)} required placeholder="Repita a senha" />
        {error && <p style={{ color:'#ef4444', fontSize:'.85rem', marginBottom:12 }}>{error}</p>}
        <Btn type="submit" fullWidth loading={loading}>Criar conta</Btn>
      </form>
      <p style={{ marginTop:20, textAlign:'center', fontSize:'.85rem', color:'#64748b' }}>
        Já tem conta?{' '}
        <Link to={plan ? `/login?plan=${plan}` : '/login'} style={{ color:'#3C5A99' }}>Entrar</Link>
      </p>
    </AuthCard>
  );
}
...
function mapError(code) {
  ...
  return map[code] || 'Erro ao criar conta. Tente novamente.';
}
